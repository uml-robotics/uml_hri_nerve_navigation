...
vector<geometry_msgs::PoseStamped> robot_map_poses;
...
void map_pose_callback(geometry_msgs::PoseStamped map_pose_msg) {
    bool map_pose_found = false;
    for (int i = 0; i < robot_map_poses.size(); ++i) {
        if (robot_map_poses[i].header.frame_id == map_pose_msg.header.frame_id) {
            robot_map_poses[i] = map_pose_msg;
            map_pose_found = true;
            break;
        }
    }
    if (!map_pose_found) {
        robot_map_poses.push_back(map_pose_msg);
    }
}

void checkGoalsStatus(vector<geometry_msgs::PoseStamped> map_pose_vector, vector<geometry_msgs::PoseStamped> goal_vector) {
    ROS_WARN("CHECKER_NODE: COMPARING TWO POSITIONS");
    bool found;
    if (robots_reached.size() == map_pose_vector.size()){
        kill_node = true;
    }
    for(int i = 0; i < map_pose_vector.size(); ++i){

        cout << "CHECKER_NODE: ODOM: " << map_pose_vector[i].pose.position.x << ", GOAL: " << goal_vector[i].pose.position.x << endl;
        ROS_INFO("MAP_POSE: %f, %f | GOAL_POSE: %f, %f", map_pose_vector[i].pose.position.x, map_pose_vector[i].pose.position.y, goal_vector[i].pose.position.x, goal_vector[i].pose.position.y);
        found = false;
        if (abs(map_pose_vector[i].pose.position.x - goal_vector[i].pose.position.x) <= 0.5
         && abs(map_pose_vector[i].pose.position.y - goal_vector[i].pose.position.y) <= 0.5
         && abs(map_pose_vector[i].pose.orientation.w - goal_vector[i].pose.orientation.w) < 0.02) {
            for (int j = 0; j < robots_reached.size(); ++j){
                if (robots_reached[j] == map_pose_vector[i].header.frame_id){
                    found = true;
                }
            }
            if (!found){
                string temp;
                stringstream ss(map_pose_vector[i].header.frame_id);
                getline(ss, temp, '/');
                cout << temp << " successfully reached its goal...." << endl;
                robots_reached.push_back(map_pose_vector[i].header.frame_id);
            }
        }
    }
}
...
int main(int argc, char** argv){
    ...
    while(ss.good()){
        ...
        map_pose_topic_name = temp + "/map_pose";
        ...
        map_pose_subs.push_back(nh.subscribe(map_pose_topic_name, 1, map_pose_callback));
    }

    ros::Rate rate(1);
    while(ros::ok()){
        // if the odoms and goal_pose are subscribed then the odom and goal_pose vector are sorted (makes it easy for comparing)
        if (!vectors_sorted && robot_odoms.size() == map_pose_subs.size()){
            ...
            sort(robot_map_poses.begin(), robot_map_poses.end(), checkGreaterGoal);

            vectors_sorted = true;
        }
        else if (vectors_sorted){
            // checkGoalsReached(robot_odoms, robot_goals);
            checkGoalsStatus(robot_map_poses, robot_goals);
